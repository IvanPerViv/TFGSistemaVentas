 ...
 * @author Iván Pérez
 */
public class Con_familias_articulos {

    protected Conexion objConexion;
    protected Connection con;

    public Con_familias_articulos() {
    ...
    public ArrayList mostrarArticulosYBusqueda(String buscar) {
        ArrayList<FamiliaArticulos> famArt = new ArrayList<>();
        ...
        try (PreparedStatement pst = con.prepareStatement(query)) {
            try (ResultSet rs = pst.executeQuery()) {
                while (rs.next()) {
                    ...
                    famArt.add(new FamiliaArticulos(cod, nombrePoc));
                }
            }
        } catch (SQLException ex) {
            System.err.println("¡Error al ejecutar la consulta!" + ex.toString());
