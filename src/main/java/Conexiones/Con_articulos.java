...
public class Con_articulos {
    ...
    public boolean ingresoDeArticulos(int cod_articulo, String nombre_producto, double precio_compra, int stock) {
        String query = "INSERT INTO `articulos`(`cod_articulo`, `nombre_producto`, `precio_compra`, `stock`) VALUES (?,?,?,?)";
        int comprobacion = 0;
    ...
    public boolean actualizarArticulos(int cod_art, String nomProc, String precioCompra, String stock) {
        int comprobacion = 0;
    ...
    public ArrayList mostrarArticulosYBusqueda(String buscar) {
        ...
        String query = "SELECT * FROM `articulos` where concat(`cod_articulo`, `nombre_producto`, `precio_compra`, `stock`) like'%" + buscar + "%'";
        try (PreparedStatement pst = con.prepareStatement(query)) {
            try (ResultSet rs = pst.executeQuery()) {
                while (rs.next()) {
                    ...
                    art.add(new Articulos(cod, nombrePoc, precioCompra, stock));
                }
            }
        } catch (SQLException ex) {
            System.err.println("¡Error al ejecutar la consulta!" + ex.toString());
